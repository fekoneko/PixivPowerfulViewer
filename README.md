...
It was made specifically for my needs, and because I really don't know how to spend my time properly. This thing kept breaking with each new package added...
...
winrt::fire_and_forget ReactNativeModule::moveFile(std::wstring filepath, std::wstring destpath, JSValueObject options, ReactPromise<void> promise) noexcept
try
{
...
winrt::fire_and_forget ReactNativeModule::copyFile(std::wstring filepath, std::wstring destpath, JSValueObject options, ReactPromise<void> promise) noexcept
try
{
    winrt::hstring srcDirectoryPath, srcFileName;
    splitPath(filepath, srcDirectoryPath, srcFileName);
...
try
{
    ...
    path.make_preferred();
    StorageFolder targetDirectory{ co_await StorageFolder::GetFolderFromPathAsync(path.wstring()) };
...
winrt::fire_and_forget ReactNativeModule::writeFile(std::wstring filepath, std::string base64Content, JSValueObject options, ReactPromise<void> promise) noexcept
try
{
...
winrt::fire_and_forget ReactNativeModule::appendFile(std::wstring filepath, std::string base64Content, ReactPromise<void> promise) noexcept
try
{
    size_t fileLength = filepath.length();
    bool hasTrailingSlash{ filepath[fileLength - 1] == '\\' || filepath[fileLength - 1] == '/' };
...
winrt::fire_and_forget ReactNativeModule::write(std::wstring filepath, std::string base64Content, int position, ReactPromise<void> promise) noexcept
try
{
    winrt::hstring directoryPath, fileName;
    splitPath(filepath, directoryPath, fileName);
...
void ReactNativeModule::splitPath(const std::wstring& fullPath, winrt::hstring& directoryPath, winrt::hstring& fileName) noexcept
{
...
std::filesystem::path ReactNativeModule::convertPath(const std::filesystem::path originalPath)
{
...
- Next, go to `node_modules\@dr.pogodin\react-native-fs\windows\ReactNativeFs\ReactNativeModule.h`
- Paste the following code there:
...
struct ReactNativeModule
{
    ...
    winrt::fire_and_forget exists(std::wstring fullpath, ReactPromise<bool> promise) noexcept;
